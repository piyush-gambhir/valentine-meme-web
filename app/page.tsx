...
type NoMode = 'button' | 'split' | 'fake' | 'loading' | 'slider' | 'checkbox' | 'link' | 'draggable';
...
const ENDING_VARIANTS: Array<{ title: string; subtitle: string; style: EndingStyle }> = [
  ...
  { title: 'CONGRATULATIONS! 🏆', subtitle: 'You found the only correct answer!', style: 'corporate' },
  { title: 'DESTINY ACCEPTED ✨', subtitle: 'The stars aligned for this moment 💫', style: 'dramatic' },
...
function Bear({ mood, size = 'normal' }: { mood: Mood; size?: 'small' | 'normal' | 'large' }) {
  ...
  return (
    <svg viewBox="0 0 200 200" className={`${sizeClasses[size]} transition-all duration-300`} xmlns="http://www.w3.org/2000/svg">
      {/* Ears */}
      <circle cx="55" cy="45" r="28" fill="#C4956A" />
      <circle cx="145" cy="45" r="28" fill="#C4956A" />
      <circle cx="55" cy="45" r="16" fill="#EDBBAB" />
      <circle cx="145" cy="45" r="16" fill="#EDBBAB" />
      
      {/* Face */}
      <circle cx="100" cy="100" r="60" fill="#C4956A" />
      <ellipse cx="100" cy="112" rx="38" ry="32" fill="#E8D0B8" />
      
      {/* Blush */}
      <ellipse cx="55" cy="112" rx="12" ry="7" fill="#FF9BB3" opacity="0.55" />
      <ellipse cx="145" cy="112" rx="12" ry="7" fill="#FF9BB3" opacity="0.55" />
      
      {/* Eyes */}
      <BearEyes mood={mood} />
      
      {/* Nose */}
      <ellipse cx="100" cy="108" rx="8" ry="6" fill="#4A3728" />
      <ellipse cx="97" cy="106" rx="3" ry="2" fill="#6B5242" opacity="0.5" />
...
function BearEyes({ mood }: { mood: Mood }) {
    ...
    return (
      <>
        <g transform="translate(72,80) scale(0.55)">
          <path d="M12 4C12 4 9.5 0 6 0C2.5 0 0 3 0 6.5C0 13 12 20 12 20S24 13 24 6.5C24 3 21.5 0 18 0C14.5 0 12 4 12 4Z" fill="#FF4081"/>
        </g>
  ...
  if (mood === 'hopeful') {
    return (
      <>
        <ellipse cx="80" cy="90" rx="9" ry="11" fill="#2D1B14" />
        <ellipse cx="120" cy="90" rx="9" ry="11" fill="#2D1B14" />
        <circle cx="84" cy="86" r="5" fill="white" />
        <circle cx="124" cy="86" r="5" fill="white" />
        <circle cx="77" cy="92" r="2.5" fill="white" opacity="0.5" />
        <circle cx="117" cy="92" r="2.5" fill="white" opacity="0.5" />
      </>
    );
  }
  ...
  if (mood === 'worried') {
    return (
      <>
        <ellipse cx="80" cy="90" rx="7" ry="9" fill="#2D1B14" />
        <ellipse cx="120" cy="90" rx="7" ry="9" fill="#2D1B14" />
        <circle cx="83" cy="87" r="3" fill="white" />
        <circle cx="123" cy="87" r="3" fill="white" />
        <line x1="67" y1="76" x2="85" y2="72" stroke="#4A3728" strokeWidth="2.5" strokeLinecap="round" />
        <line x1="133" y1="76" x2="115" y2="72" stroke="#4A3728" strokeWidth="2.5" strokeLinecap="round" />
      </>
    );
  }
  ...
  if (mood === 'crying') {
    return (
      <>
        <ellipse cx="80" cy="90" rx="7" ry="9" fill="#2D1B14" />
        <ellipse cx="120" cy="90" rx="7" ry="9" fill="#2D1B14" />
        <circle cx="83" cy="86" r="3" fill="white" />
        <circle cx="123" cy="86" r="3" fill="white" />
        <line x1="67" y1="74" x2="85" y2="70" stroke="#4A3728" strokeWidth="2.5" strokeLinecap="round" />
        <line x1="133" y1="74" x2="115" y2="70" stroke="#4A3728" strokeWidth="2.5" strokeLinecap="round" />
        <ellipse cx="66" cy="105" rx="4" ry="8" fill="#7EC8E3" opacity="0.6" className="animate-tear-l" />
        <ellipse cx="134" cy="105" rx="4" ry="8" fill="#7EC8E3" opacity="0.6" className="animate-tear-r" />
      </>
    );
  }
  ...
  return (
    <>
      <line x1="72" y1="82" x2="88" y2="98" stroke="#2D1B14" strokeWidth="4" strokeLinecap="round" />
      <line x1="88" y1="82" x2="72" y2="98" stroke="#2D1B14" strokeWidth="4" strokeLinecap="round" />
      <line x1="112" y1="82" x2="128" y2="98" stroke="#2D1B14" strokeWidth="4" strokeLinecap="round" />
      <line x1="128" y1="82" x2="112" y2="98" stroke="#2D1B14" strokeWidth="4" strokeLinecap="round" />
      <ellipse cx="64" cy="108" rx="5" ry="10" fill="#7EC8E3" opacity="0.5" className="animate-tear-l" />
      <ellipse cx="136" cy="108" rx="5" ry="10" fill="#7EC8E3" opacity="0.5" className="animate-tear-r" />
    </>
  );
}
...
function BearMouth({ mood }: { mood: Mood }) {
  ...
  return <path d={paths[mood]} fill="none" stroke="#4A3728" strokeWidth="3" strokeLinecap="round" />;
}
...
function Confetti({ active, density = 'normal' }: { active: boolean; density?: 'low' | 'normal' | 'high' }) {
...
function MiniConfetti({ count, centerRef }: { count: number; centerRef: React.RefObject<HTMLElement | null> }) {
  const [positions, setPositions] = useState<Array<{ x: number; y: number; emoji: string; delay: number }>>([]);
  ...
  return (
    <>
      {positions.map((pos, i) => (
        <span
          key={i}
          className="absolute pointer-events-none animate-mini-confetti"
          style={{
            left: `${pos.x}%`,
            top: `${pos.y}%`,
  ...
  return (
    <>
      {positions.map((pos, i) => (
        <span
          key={i}
          className="pointer-events-none fixed z-20 animate-fade-out text-lg"
          style={{
            left: pos.x,
            top: pos.y,
...
function NoClone({ x, y, scale, rotation, label, onClick }: {
  x: number;
  y: number;
  scale: number;
  rotation: number;
  ...
  return (
    <button
      onClick={onClick}
      className="fixed z-30 rounded-xl border-2 border-rose-200 bg-white/90 px-5 py-2.5 font-hand text-rose-600 shadow-md backdrop-blur-sm transition-transform hover:scale-105"
      style={{
        left: x,
        top: y,
...
export default function ValentinePage() {
  ...
  const teleportNo = useCallback((isPanic = false) => {
    ...
    // Rotation
    const rotMag = stage === 'boss' ? 30 : stage === 'chaos' ? 20 : stage === 'persuasion' ? 10 : 5;
  ...
  // Handle No click
  const handleNoClick = useCallback((e?: React.MouseEvent | React.TouchEvent | undefined) => {
    e?.preventDefault?.();
    e?.stopPropagation?.();
  ...
  const handleNoMouseDown = useCallback((e: React.MouseEvent) => {
  ...
  const renderNoButton = () => {
    ...
    switch (noMode) {
      case 'split':
        return (
          <div className={`${isFixed ? 'fixed' : 'relative'} z-40 flex`} style={isFixed ? { left: noPos.x, top: noPos.y } : {}}>
            <button 
              className="rounded-l-xl border-2 border-rose-200 bg-white px-4 py-2 font-hand text-rose-600 animate-split-left"
              onClick={() => handleNoClick()}
            >
              N
            </button>
            <button 
              className="rounded-r-xl border-2 border-rose-200 bg-white px-4 py-2 font-hand text-rose-600 animate-split-right"
              onClick={() => handleNoClick()}
            >
              o
            </button>
      ...
      case 'fake':
        return (
          <div 
            className={`${isFixed ? 'fixed' : 'relative'} z-40 rounded-xl bg-gray-100 px-6 py-3 font-hand text-gray-400`}
            style={commonStyles}
          >
            No (disabled)
          </div>
        ...
        return (
          ...
          >
            ...
            <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-rose-500 transition-all duration-100" style={{ width: `${loadingProgress}%` }} />
            </div>
      ...
      case 'slider':
        return (
          ...
          >
            <label className="mb-2 block text-center text-sm font-hand text-rose-600">
              Slide to No
            </label>
            <input
              type="range"
      ...
      case 'checkbox':
        return (
          <label 
            className={`${isFixed ? 'fixed' : 'relative'} z-40 flex items-center gap-2 rounded-xl bg-white/90 px-4 py-3 shadow-lg backdrop-blur-sm cursor-pointer`}
            style={commonStyles}
          >
            <input 
              type="checkbox" 
      ...
      case 'draggable':
        return (
          <button
            ref={noBtnRef}
            className={`${isFixed ? 'fixed' : 'relative'} z-40 font-hand rounded-xl border-2 border-rose-200 bg-white px-6 py-3 text-rose-700 shadow-sm active:cursor-grabbing transition-all duration-150 ${fakeDisabled ? 'opacity-50 grayscale' : ''}`}
            style={commonStyles}
      ...
      default:
        return (
          <button
            ...
            onFocus={handleNoFocus}
            className={`${isFixed ? 'fixed' : 'relative'} z-40 font-hand rounded-xl border-2 border-rose-200 bg-white px-6 py-3 text-rose-700 shadow-sm transition-all duration-150 hover:bg-rose-50 ${
              stage === 'playful' ? 'animate-tilt' :
              stage === 'persuasion' ? 'animate-tremble' :
              stage === 'chaos' ? 'animate-spin-slow' :
              stage === 'boss' ? 'animate-panic' : ''
          ...
          >
            ...
            {attempts >= 4 && (
              <span className="pointer-events-none absolute inset-0 -m-6 rounded-full border-2 border-rose-200/40 animate-pulse-ring" />
            )}
            
            {/* Fake lag indicator */}
            {showFakeLag && (
              <span className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-yellow-400 px-2 py-0.5 text-xs text-yellow-900 animate-bounce">
                Loading...
              </span>
            )}
          </button>
  ...
  if (hasAccepted) {
    ...
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center overflow-hidden bg-gradient-party">
        <Confetti active={true} density="high" />
        
        <div className="z-10 flex flex-col items-center gap-8 px-6 text-center animate-fade-scale max-w-2xl">
          {/* Big celebration bears */}
          <div className="flex items-center justify-center gap-4">
            <div className="animate-bounce-gentle">
              <Bear mood="love" size="large" />
            </div>
            <div className="flex flex-col items-center">
              <span className="text-7xl animate-pulse">💞</span>
              <span className="text-4xl mt-2">🎉</span>
            </div>
            <div className="-scale-x-100 animate-bounce-gentle" style={{ animationDelay: '0.2s' }}>
              <Bear mood="love" size="large" />
            </div>
          </div>
          ...
          <div className="relative">
            <h1 className={`font-display text-6xl font-black text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.5)] sm:text-8xl ${ending.style === 'anime' ? 'animate-bounce' : ''}`}>
              {ending.title}
            </h1>
            <div className="absolute -inset-4 bg-white/20 blur-3xl -z-10 rounded-full" />
          </div>
          
          {/* Subtitle */}
          <p className="font-hand text-3xl text-white/95 sm:text-4xl">
            {ending.subtitle}
          </p>
          
          {/* Message card */}
          <div className="mt-6 w-full max-w-lg rounded-[2rem] bg-white/25 px-10 py-8 backdrop-blur-xl border-2 border-white/40 shadow-2xl">
  ...
  return (
    <div 
      ...
      style={{
        background: 'linear-gradient(135deg, #FF9A9E 0%, #FAD0C4 25%, #FFB6C1 50%, #FF69B4 75%, #FF4081 100%)',
    ...
    >
      ...
      {safeZoneVisible && yesBtnRef.current && (
        <div 
          className="pointer-events-none fixed border-2 border-dashed border-yellow-400/50 rounded-2xl z-10 animate-pulse"
          style={{
      ...
      <div 
        ref={cardRef}
        className={`relative z-20 flex w-full max-w-md flex-col items-center gap-5 rounded-[2.5rem] bg-white/95 px-8 pb-10 pt-12 shadow-2xl backdrop-blur-sm transition-all duration-500 ${ready ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
        style={{
      ...
      >
        {/* Notifications */}
        {devToolsOpen && (
          <div className="absolute -top-14 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-gray-800 px-4 py-1.5 text-sm text-white animate-fade-in shadow-lg">
            I saw that. 👀
          </div>
        )}
        
        {windowResized && (
          <div className="absolute -top-14 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-red-500 px-4 py-1.5 text-sm text-white font-bold animate-pop shadow-lg">
            Hey, don&apos;t cheat! 🛑
          </div>
        )}
        
        {showAchievement && (
          <div className="absolute -top-20 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-yellow-400 px-5 py-2 font-bold text-yellow-900 shadow-xl animate-pop z-50">
            {showAchievement}
          </div>
        )}
        
        {longPressToast && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 rounded-full bg-blue-500 px-4 py-1.5 text-sm text-white animate-fade-in">
            Nice try! 📱
          </div>
        )}
        ...
        {konamiProgress > 0 && konamiProgress < 10 && (
          <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex gap-1 bg-black/20 px-3 py-1 rounded-full">
            {Array.from({ length: konamiProgress }).map((_, i) => (
              <span key={i} className="text-lg animate-pop">⬆️</span>
            ))}
            <span className="text-xs text-white/70 ml-1">{konamiProgress}/10</span>
          </div>
        )}
        ...
        {showUltraYes && (
          <div className="absolute -top-16 left-1/2 -translate-x-1/2 animate-pulse">
            <span className="rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 px-5 py-2 font-bold text-white shadow-xl text-sm">
              🌟 ULTRA YES MODE 🌟
            </span>
          </div>
        )}
        ...
        {showDifficulty && (
          <div className="absolute top-3 right-4 flex gap-2 text-xs font-medium">
            <button className="px-2 py-1 rounded bg-gray-200 text-gray-500 line-through">Easy</button>
            <button className="px-2 py-1 rounded bg-pink-200 text-pink-700 animate-pulse">Medium</button>
            <button className="px-2 py-1 rounded bg-gray-200 text-gray-500 line-through">Hard</button>
          </div>
        )}
        ...
        {idleTime > 0 && idleTime < 4 && stage !== 'intro' && (
          <div className="absolute -top-12 left-1/2 -translate-x-1/2 animate-fade-in">
            <p className="font-hand text-lg text-rose-600 bg-white/80 px-4 py-1 rounded-full shadow-sm">
        ...
        <div 
          className={`transition-all duration-500 ${
            attempts === 0 ? 'animate-bounce-gentle' : 
            attempts < 7 ? 'animate-wobble' : 
            'animate-tremble'
          }`}
          style={{
        ...
        >
          <Bear mood={mood} size="normal" />
        </div>
        
        {/* Title */}
        <h1 
          className="font-display text-center text-2xl font-black leading-tight tracking-tight text-gray-900 sm:text-3xl cursor-pointer select-none"
        ...
        {/* Analytics */}
        {attempts >= 4 && (
          <div className="flex gap-6 text-xs font-medium text-gray-500 animate-fade-in bg-gray-100 px-4 py-2 rounded-full">
        ...
        {/* Countdown */}
        {attempts >= 9 && (
          <div className="rounded-full bg-gradient-to-r from-red-100 to-pink-100 px-5 py-2 text-sm font-bold text-red-600 animate-pulse border border-red-200">
        ...
        {attempts >= 6 && (
          <div className="text-center animate-fade-in">
            <p className="font-hand text-base text-purple-600">
              Combo: {combo} misses! {combo >= 15 ? '🔥🔥🔥' : combo >= 10 ? '🔥🔥' : combo >= 5 ? '🔥' : ''}
            </p>
            {comboRank && (
              <p className="text-xs text-purple-500 font-bold mt-0.5">{comboRank}</p>
            )}
          </div>
        )}
        ...
        {showCaptcha && (
          <div className="animate-pop rounded-xl border-2 border-gray-300 bg-gray-50 p-5 text-center shadow-inner">
            <p className="mb-3 text-sm font-medium text-gray-700">Select all squares with &quot;No&quot;</p>
        ...
        {/* Yes Buttons Area */}
        <div className={`mt-2 flex w-full flex-col items-center gap-4 ${isFullWidth ? 'px-0' : ''}`}>
          {/* Extra Yes copies */}
          {yesCopies > 0 && (
            <div className="flex flex-wrap justify-center gap-3">
              {Array.from({ length: yesCopies }).map((_, i) => (
                <button
                  ...
                  className="rounded-xl bg-gradient-to-r from-pink-400 to-rose-500 px-5 py-2.5 text-sm font-bold text-white shadow-lg transition-all hover:scale-110 active:scale-95 animate-bounce-gentle"
          ...
          <button
            ref={yesBtnRef}
            onClick={handleYesClick}
            className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-pink-500 via-rose-500 to-red-500 font-bold text-white shadow-2xl transition-all hover:shadow-pink-500/50 active:scale-95 group"
            style={{
          ...
          >
            <span className="relative z-10 flex items-center gap-2 font-hand">
              {yesText}
            </span>
            
            {/* Confetti around Yes */}
            {attempts >= 5 && !isReducedMotion && (
              <MiniConfetti count={Math.min(attempts - 4, 10)} centerRef={yesBtnRef} />
            )}
            ...
            {/* Pulse ring */}
            {attempts >= 3 && (
              <span className="absolute inset-0 rounded-2xl animate-pulse-ring bg-white/20" />
            )}
          </button>
        </div>
        ...
        {attempts >= 18 && (
          <button
            onClick={handleNoClick}
            className="fixed bottom-4 text-xs text-gray-400 underline decoration-dotted hover:text-gray-600 animate-bounce-gentle transition-colors"
            style={{
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="animate-pop rounded-2xl bg-white p-8 text-center shadow-2xl max-w-sm mx-4">
            <h3 className="mb-2 font-display text-2xl font-bold text-gray-900">Are you sure?</h3>
            ...
            <div className="flex gap-3 justify-center">
              <button
                ...
                className="rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 px-8 py-3 font-bold text-white shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95"
              >
                Yes! 💖
              </button>
              <button
                ...
                className="rounded-xl bg-gray-200 px-8 py-3 font-bold text-gray-700 hover:bg-gray-300 transition-all hover:scale-105 active:scale-95"
              >
                Also Yes
              </button>
            </div>
      ...
      {stage === 'boss' && attempts === 15 && (
        <div className="fixed top-20 z-50 animate-boss-appear pointer-events-none">
          <h2 className="font-display text-5xl font-black text-red-600 drop-shadow-2xl text-center">
            ⚔️ FINAL BOSS: NO ⚔️
          </h2>
          <p className="text-center text-red-500 font-bold mt-2">IMPOSSIBLE DIFFICULTY</p>
        </div>
      )}
    </div>
  );
